...
const PostProcessing = ({
  ...
  const render = useMemo(() => {
    ...
    const pinkOverlay = vec3(1.0, 0.0, 0.5).mul(oneMinus(scanLine)).mul(0.4);

    // Mix the original scene with the pink overlay
...
const Scene = () => {
  ...
  const { material, uniforms } = useMemo(() => {
    ...
    const mask = dot.mul(flow).mul(vec3(10.0, 0.0, 5.0));
...
export const HeroFuturistic = () => {
  ...
  const subtitle = 'Lapidando ideias. Brilho inesquecível.';
  ...
  return (
    <div className="w-full h-svh relative overflow-hidden bg-deep-black">
